...
contract PledgePool is ReentrancyGuard, SafeTransfer, multiSignatureClient{  //防重入攻击，安全转账，多重签名钱包合约
    ...
    uint256 public borrowFee;
        ...
        // 每个池的基本信息
    struct PoolBaseInfo{
        uint256 settleTime;         // 结算时间  完成抵押 开始执行借贷
        uint256 endTime;            // 结束时间  贷款到期清算时间，出借方拿走本金和利息,借款人拿走抵押物(根据当时价格卖掉资产,支付出借方的本金、利息、手续费)
        uint256 interestRate;       // 池的固定利率，单位是1e8 (1e8) 
        uint256 maxSupply;          // 池的最大限额  投资资金的最大上限
        uint256 lendSupply;         // 当前实际存款的借款
        uint256 borrowSupply;       // 当前实际存款的借款
        uint256 martgageRate;       // 池的抵押率，单位是1e8 (1e8)
        address lendToken;          // 借款方代币地址 (比如 BUSD.. 稳定币等)  
        address borrowToken;        // 借款方代币地址 (比如 BTC、ETH.. 抵押币)
        PoolState state;            // 状态 'MATCH, EXECUTION, FINISH, LIQUIDATION, UNDONE'
        IDebtToken spCoin;          // sp_token的erc20地址 (比如 spBUSD_1..)  sp_token抵押方拿到的抵押凭证
        IDebtToken jpCoin;          // jp_token的erc20地址 (比如 jpBTC_1..)   jp_token 投资人拿到的投资凭证
    ...
    constructor(
        address _oracle,   //获取抵押资产的价格
        address _swapRouter,   //链上 Dex上卖资产时 Router就是Dex的入口, (像以太坊上的uniswap, bsc上的pancakeswap)
        address payable _feeAddress,  //协议费用地址 授权的管理员将协议费转入这个地址
        address _multiSignature   //多签地址
     ...
     *  Can only be called by the owner. 调用createPoolInfo 创建之后池子就是match状态，可以开始出借和借入，且需要在设定的时间之前
     */
    ...
    function getPoolState(uint256 _pid) public view returns (uint256) {
       ...
       /**  timeBefore 结束时间之前 进行操作
     * @dev 存款人执行存款操作
     * @notice 池状态必须为MATCH stateMatch
     * @param _pid 是池索引
    ...
    function refundLend(uint256 _pid) external nonReentrant notPause timeAfter(_pid) stateNotMatchUndone(_pid){
     ...
     /**
     * @dev 存款人接收 sp_toke,主要功能是让存款人领取 sp_token (存款凭证)
     * @notice 池状态不等于匹配和未完成
     * @param _pid 是池索引
     */
     ...
     * @param _pid 是池子的索引
     */
    function settle(uint256 _pid) public validCall { // 验证调用者 多签钱包授权的地址
        // 获取基础池信息
        PoolBaseInfo storage pool = poolBaseInfo[_pid];
        // 获取数据池信息
        PoolDataInfo storage data = poolDataInfo[_pid];
