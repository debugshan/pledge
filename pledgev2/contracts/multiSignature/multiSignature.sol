...
//多签钱包 区别于gnosis钱包, 针对每一笔交易的签名， 多个钱包发起一笔交易时,多个钱包对交易签名，然后两个签名放到一起传给这个gnosis合约,合约校验两个签名是否在多签人的地址列表里, 校验通过由合约执行交易。
...
uint256 private constant defaultIndex = 0;
    ...
    //key: 调用方 + 合约地址 的哈希 ==>  value: 签名者数组 (针对合约地址的签名者列表)
    mapping(bytes32=>signatureInfo[]) public signatureMap; //签给合约,对 某个人对于某个合约的权限做签署, 授予某人对某个合约的权限
    ...
    // 多签钱包：钱包三个人（owners地址）共有时，有一个门限（limitedSignNum=2）,比如满足两人签名,即可发起交易。
    ...
    function createApplication(address to) external returns(uint256) { 
        bytes32 msghash = getApplicationHash(msg.sender,to); //调用方 + 合约地址 的哈希
        uint256 index = signatureMap[msg hash].length;     // 获取签名数量，获取签名者数组的长度(第一次应该是0)
    ...
    function signApplication(bytes32 msghash) external onlyOwner validIndex(msghash,defaultIndex){
        ...
        signatureMap[msghash][defaultIndex].signatures.addWhiteListAddress(msg.sender);  //添加到白名单列表中
    }
    //撤销签名
